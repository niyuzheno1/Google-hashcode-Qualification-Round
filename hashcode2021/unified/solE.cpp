...
int main()
{
    ...
    for(int i = 0; i < V; ++i){
        ...
        int DP = P-1;
        while(P--){
            string s; cin >> s;vehi[i].seq.push_back(s);
            if(P == DP) continue;
    ...
    sort(vehi, vehi + V, [] (const vehicle & lhs, const vehicle & rhs) {return lhs.TL < rhs.TL;} );
    for(int i = 0; i < V; ++i){
        int TL = 0;
        for(int j = 0; j < vehi[i].seq.size(); ++ j){
            auto s = vehi[i].seq[j];
            //if(visited[endEdge[s]]) TL = -1;
        }
        //if(TL == -1) continue;
        for(int j = 0; j < vehi[i].seq.size(); ++ j){
            auto s = vehi[i].seq[j];
